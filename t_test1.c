...
{
	struct _pthread_v *tv = calloc1(1, sizeof(struct _pthread_v));
...
// #endif
	...
	p.m = malloc1(st->bins * sizeof(*p.m));
	p.bins = st->bins;
	p.size = st->size;
	for (b = 0; b < p.bins; b++)
	{
...
#elif USE_THR
	if (!st->sp) st->sp = malloc1(STACKSIZE);
	if (!st->sp) return -1;
